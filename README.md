...
- 결론 내기: 사용자가 "결론 내리기" 버튼으로 언제든 요청할 수 있고(토론 중이면 현재 발언이 끝난 뒤),
  진행 도우미가 5턴마다 "지금 정리할 만한가"를 스스로 판단해 결론을 냅니다.
  결론은 대화에 남고 아이디어 지도에 `conclusion` 노드로 고정됩니다.
...
| `CONCLUSION_CHECK_INTERVAL` | 진행 도우미가 결론 가능성을 판단하는 주기(AI 턴). `0`이면 자동 결론 없음 | `5` |
...
- 에이전트 시스템 프롬프트에는 *성향*(아이디어 제시, 비판, 대안, 실현 가능성, 엉뚱한 발상)과
  말투 규칙(아주 쉬운 한국어, 짧은 문장, 전문용어·영어 약어·비즈니스 유행어 금지)만 담깁니다 —
  대본이나 순서는 결코 넣지 않습니다.
- `CONCLUSION_CHECK_INTERVAL`(기본 5) AI 턴마다 진행 도우미가 "지금 결론을 낼 만큼 무르익었나"를
  판단하고, 그렇다고 보면 결론을 써서 토론을 끝냅니다. 사용자가 "결론 내리기"를 누르면
  진행 중인 토론은 현재 발언이 끝나는 즉시, 멈춰 있는 토론은 바로 결론을 냅니다.
